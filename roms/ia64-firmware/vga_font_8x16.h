/*
 * Standard IBM/VGA 8x16 CP437 text font.
 *
 * Extracted verbatim from the Bochs VGABIOS 8x16 character table
 * (qemu pc-bios/vgabios-stdvga.bin).  A raw bitmap font carries no
 * copyright, and the Bochs VGABIOS is LGPL, compatible with this GPL
 * firmware.  256 glyphs, 16 bytes each; one byte per scan line, bit 0x80
 * is the leftmost pixel.  This is the font the real i2000 VGA character
 * generator renders from plane 2, so loading it there (or rasterising it
 * in the graphics console) matches real hardware.
 */
#ifndef IA64_FW_VGA_FONT_8X16_H
#define IA64_FW_VGA_FONT_8X16_H

#define VGA_FONT_8X16_GLYPHS 256
#define VGA_FONT_8X16_HEIGHT 16

static const unsigned char gVgaFont8x16[VGA_FONT_8X16_GLYPHS][VGA_FONT_8X16_HEIGHT] = {
